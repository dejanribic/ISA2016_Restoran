...
(function () {
    ...
    function MenuItemController($cookies, $scope, $http, $location) {
        ...
        $scope.initMap = function () {
            var latLong = $scope.restaurant.coordinates.split(",");
        ...
        $scope.removeMenuItem = function (toRemove) {

            $http.put('/menu-item/remove', toRemove.menuItem).success(reloadMenuItems);
        };
        $scope.updateRestaurant = function () {
            var coords = String($scope.coords['lat'])+','+String($scope.coords['lng']);
            $scope.restaurant.coordinates = coords;
            $http.put('/restaurants/create', $scope.restaurant).success(function (response) {alert("Info updated")});
        };
